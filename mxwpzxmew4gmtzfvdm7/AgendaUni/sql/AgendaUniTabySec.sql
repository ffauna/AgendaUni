...
CREATE TABLE USUARIOS (
    ...
    TIPOUSUARIO    VARCHAR2(6) CHECK(TIPOUSUARIO IN ('ALUMNO','PDI','PAS')) ,
...
CREATE TABLE CARGOSACADEMICOS (
    OID_CA    INTEGER    NOT NULL,
    OID_U        INTEGER    NOT NULL,    
    NOMBRECARGO    VARCHAR2(256),
PRIMARY KEY (OID_CA),
FOREIGN KEY (OID_U) REFERENCES USUARIOS);
...
START WITH 1 INCREMENT BY 1 NOMAXVALUE;
CREATE SEQUENCE SEC_ADMINS 
START WITH 1 INCREMENT BY 1 NOMAXVALUE;
CREATE SEQUENCE SEC_CARGOSACADEMICOS 
START WITH 1 INCREMENT BY 1 NOMAXVALUE;
...
/*Triggers*/
/
CREATE OR REPLACE TRIGGER C_PERSONAL_ADMIN_O_CARGOA 
...
IF :NEW.TIPOCALENDARIO != 'OFICIAL' AND (TIPO = 'PAS' OR TIPO = 'PDI') THEN
    RAISE_APPLICATION_ERROR
      (-20602,'Un usuario tipo PDI o tipo PAS solo puede crear calendarios tipo OFICIAL');
END IF;
END;
/
...
END;
/
CREATE OR REPLACE TRIGGER CREA_CARGACADEM_SI_PDI 
AFTER INSERT OR UPDATE OF TIPOUSUARIO ON USUARIOS
FOR EACH ROW
BEGIN
  IF :NEW.TIPOUSUARIO = 'PDI' THEN
    CREA_CARGOACADEMICO(:NEW.OID_U, :NEW.NOMBRE);
  END IF;
END;
/
...
END CREA_ADMINS;
/
CREATE OR REPLACE PROCEDURE CREA_CARGOACADEMICO 
    (OID_U IN CARGOSACADEMICOS.OID_U%TYPE,
    NOMBRECARGO IN CARGOSACADEMICOS.NOMBRECARGO%TYPE) AS 
BEGIN
  INSERT INTO CARGOSACADEMICOS (OID_U, NOMBRECARGO)
  VALUES(OID_U, NOMBRECARGO);
END CREA_CARGOACADEMICO;
/
...
BEGIN
  DECLARE
  CURSOR cu IS
  SELECT NOMBRE, LATITUD, LONGITUD, FECHAINICIO, FECHAFIN, TIPOEVENTO, DIACOMPLETO, DESCRIPCION FROM EVENTOS
  ORDER BY FECHAINICIO;
  BEGIN
  FOR fila IN cu LOOP
    SYS.DBMS_OUTPUT.PUT_LINE(fila.NOMBRE||' '||fila.LATITUD||' '||fila.LONGITUD||' '||fila.FECHAINICIO||' '
    ||fila.FECHAFIN||' '||fila.TIPOEVENTO||' '||fila.DIACOMPLETO||' '||fila.DESCRIPCION);
    END LOOP;
  END;
...
CREATE OR REPLACE PROCEDURE INSERTAR_EVENTO 
    ...
    W_FECHAINICIO IN VARCHAR2,
    W_FECHAFIN IN VARCHAR2,
...
BEGIN
  INSERT INTO EVENTOS VALUES(SEC_EVENTOS.nextval, w_nombre, w_latitud, w_longitud, w_fechainicio, w_fechafin, w_tipoevento,
...
CREATE OR REPLACE PROCEDURE MODIFICAR_EVENTO 
    ...
    W_FECHAINICIO IN EVENTOS.FECHAINICIO%TYPE,
    W_FECHAFIN IN EVENTOS.FECHAFIN%TYPE,
...
IF W_OID_U = D_OID_U THEN
  ...
  LONGITUD = W_LONGITUD, FECHAINICIO = W_FECHAINICIO,
  FECHAFIN = W_FECHAFIN, TIPOEVENTO = W_TIPOEVENTO, DIACOMPLETO = W_DIACOMPLETO, DESCRIPCION = W_DESCRIPCION
  WHERE OID_C = W_OID_C;
END IF;
END;
