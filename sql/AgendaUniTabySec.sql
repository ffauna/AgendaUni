...
EXEC INSERTAR_USUARIO('Juan','Pérez Martín','PAS', 'juan@us.es', 'qwerty');
OID_JUAN := SEC_USUARIOS.CURRVAL;
EXEC INSERTAR_USUARIO('Josefa','López Consuegra', 'PAS' , 'josefa@us.es', '123456');
OID_JOSE := SEC_USUARIOS.CURRVAL;
EXEC INSERTAR_USUARIO('Matilde','Arjona Gómez', 'ALUMNO', 'matilde@us.es', 'matilde');
OID_MATILDE := SEC_USUARIOS.CURRVAL;
EXEC INSERTAR_USUARIO('Luisa','García Marín', 'ALUMNO', 'luisa@us.es', 'qwerty');
OID_LUISA := SEC_USUARIOS.CURRVAL;
EXEC INSERTAR_CALENDARIO('Calendario-1','Esto es una prueba de calendario', 'NOCOMPARTIDO', 'SI', OID_JOSE); 
EXEC INSERTAR_EVENTO('Examen de IISSI', NULL, NULL, SYSDATE, NULL, NULL, 'UNICO', 'NO', NULL, OID_JOSE, SEC_CALENDARIOS.CURRVAL, NULL);